...
function decodePages(buffer, opts) {
  ...
  while (cursor.offset < cursor.size && (!opts.num_values || data.dlevels.length < opts.num_values)) {
    ...
    for (let i = 0; i < pageData.rlevels.length; i++) {
      ...
      // TODO: it need to be supported more than 18 precision
      if(opts.column.primitiveType === 'DECIMAL') {
        if(value.length >= 8) {
          value = (INT53.readInt64BE(value, value.length - 8) / Math.pow(10, opts.column.scale));
        } else if(value.length < 8) {
          value = (INT53.readInt64BE(Buffer.concat([Buffer.alloc(8-value.length, 0), value])) / Math.pow(10, opts.column.scale));
        }
      }
