...
function decodeValues_DECIMAL(cursor, count, opts) {
  let { precision, scale, typeLength } = opts;

  let values = [];

  if(typeLength >= 8) {
    values.push(INT53.readInt64BE(cursor.buffer, typeLength - 8) / Math.pow(10, scale));
  } else if(typeLength < 8) {
    values.push(INT53.readInt64BE(Buffer.concat([Buffer.alloc(8-typeLength, 0), cursor.buffer])) / Math.pow(10, scale));
  }
